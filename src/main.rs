...
use jira_dc_mcp::core::component_registry::ComponentRegistry;
use jira_dc_mcp::core::config_manager::load_config;
use jira_dc_mcp::core::config_schema::Transport;
use jira_dc_mcp::core::health_check_manager::HealthCheckManager;
use jira_dc_mcp::core::logger::init_logging;
use jira_dc_mcp::core::mcp_server::{McpifyServer, connect_stdio};
use jira_dc_mcp::core::otel;
use jira_dc_mcp::core::shutdown_handler::{install_shutdown_handlers, on_shutdown};
use jira_dc_mcp::data::store::{open_store, resolve_store_path};
use jira_dc_mcp::http::server::start_http_server;
use jira_dc_mcp::http::types::HttpServerConfig;

#[derive(Parser)]
#[command(
    name = "jira-dc-mcp",
    about = "Jira Software Data Center REST API Reference MCP server",
    version
)]
struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
enum Command {
    /// Interactively configure Jira Software Data Center REST API Reference credentials and connection settings
    Setup,
...
async fn run_harness_server() -> anyhow::Result<()> {
    ...
    let (otel_layer, otel_provider) = match otel::build_layer("jira-dc-mcp") {
    ...
    if config.transport == Transport::Http {
        ...
        let (header_location, header_name) =
            jira_dc_mcp::auth::auth_manager::header_location_for(config.auth_method);
...
async fn main() -> anyhow::Result<()> {
    ...
    let result = match cli.command {
        ...
        Command::Start => {
            ...
            unsafe {
                std::env::set_var("JIRA_DC_MCP_TRANSPORT", "stdio");
            }
        ...
        Command::Http {
            ...
            // spawns any concurrent work that would also touch env vars.
            unsafe {
                std::env::set_var("JIRA_DC_MCP_TRANSPORT", "http");
                if let Some(host) = &host {
                    std::env::set_var("JIRA_DC_MCP_HOST", host);
                }
                if let Some(port) = port {
                    std::env::set_var("JIRA_DC_MCP_PORT", port.to_string());
                }
                if let Some(cors_allow) = &cors_allow {
                    std::env::set_var("JIRA_DC_MCP_CORS_ALLOW", cors_allow);
                }
            }
            run_harness_server().await
        }
