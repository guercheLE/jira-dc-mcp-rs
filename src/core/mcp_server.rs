...
impl McpifyServer {
    ...
    #[tool(
        description = "Semantic search for Jira Software Data Center REST API Reference operations using a natural-language query."
    )]
    async fn search(
        &self,
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific Jira Software Data Center REST API Reference operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live Jira Software Data Center REST API Reference API operation, and validate the response."
    )]
    async fn call(
        &self,
...
mod tests {
    ...
    async fn search_and_get_return_mcp_content_envelopes() {
        ...
        let operation_id = {
            let conn = cached_store_connection("11.3").unwrap();
            let conn = conn
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
