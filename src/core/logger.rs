...
const LOG_LEVEL_ENV_VAR: &str = "JIRA_DC_MCP_LOG_LEVEL";
const TRANSPORT_ENV_VAR: &str = "JIRA_DC_MCP_TRANSPORT";

/// `true` when the active transport is HTTP (see `main`'s `Command::Http`
/// handling), in which case stdout carries no protocol traffic and logs go
/// there instead of stderr — see `init_logging`.
...
/// `JIRA_DC_MCP_LOG_LEVEL`, defaulting to `"info"` when unset.
...
pub fn init_logging(otel_layer: Option<BoxedLayer>) {
    ...
    // JSON-RPC over stdout — any other writer sharing that stream (tracing's
    // default) corrupts every frame parsed on the client side. Logs go to
    // stderr in stdio mode; in HTTP mode stdout carries no protocol traffic,
    // so logs go there instead (see `logs_to_stdout`).
