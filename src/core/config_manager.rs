...
const ENV_PREFIX: &str = "JIRA_DC_MCP";
const CONFIG_DIR_NAME: &str = ".jira-dc-mcp";
const LOCAL_CONFIG_FILE: &str = "jira-dc-mcp.config.yml";
...
/// (`./jira-dc-mcp.config.yml`) -> home file
/// (`~/.jira-dc-mcp/config.yml`) -> system file
/// (`/etc/jira-dc-mcp/config.yml`) -> install-dir file -> built-in
...
pub fn load_config(cli_flags: Map<String, Value>) -> Result<Config, McpifyError> {
    ...
    let layers = [
        ...
        read_yaml_if_exists(&PathBuf::from("/etc/jira-dc-mcp/config.yml")),
